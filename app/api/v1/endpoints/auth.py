...
async def login_for_access_token(
    ...
    Başarılı giriş sonrası dönen token'ı Swagger UI'da 'Authorize' kısmında kullanın.
...
async def register_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    Başarılı kayıt sonrası /token endpoint'i ile giriş yapabilirsiniz.
    """
    ...
    return {
        ...
        "user": created_user,
        "next_steps": {
            "login": "POST /token endpoint'i ile giriş yapın",
            "swagger_ui": "http://127.0.0.1:8000/docs",
            "authorize": "Swagger UI'da sağ üstteki 'Authorize' butonuna tıklayın"
        }
    }
